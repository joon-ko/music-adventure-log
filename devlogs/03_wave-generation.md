...
Yay, now we have implementations of all four wave types! To help see the audio signals we generate, I've created a module called the *wave visualizer* that functions as an *audio passthrough*: when it's processed, it simply copies its input to its output. Visually, it draws the data in its input buffer on the screen.
